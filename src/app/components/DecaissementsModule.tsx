...
export default function DecaissementsModule() {
  ...
  const amountInput = (value: number, onChange: (n: number) => void, disabled?: boolean) => (
    <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, border: '1px solid var(--border)', borderRadius: 8, padding: '6px 10px', background: disabled ? 'var(--muted)' : 'var(--input-background)' }}>
  ...
  return (
    <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: '16px', fontFamily: 'var(--font-sans)' }}>
      ...
      {/* Recherche */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '9px 12px', border: '1px solid var(--border)', borderRadius: 10, background: 'var(--card)' }}>
          ...
          <div key={c.id} style={card}>
            <button onClick={() => setExpanded(isOpen ? null : c.id)} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 14, padding: '16px 18px', background: 'transparent', border: 'none', cursor: 'pointer', textAlign: 'left', flexWrap: 'wrap' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0, flexWrap: 'wrap' }}>
                <span style={{ fontSize: '0.625rem', fontWeight: 700, padding: '3px 9px', borderRadius: 99, color: s.terrainDecaisse ? 'var(--success)' : '#C8921A', background: s.terrainDecaisse ? 'rgba(26,107,68,0.1)' : 'rgba(200,146,26,0.1)' }}>
                  {s.terrainDecaisse ? 'Terrain financé' : 'Terrain à financer'}
                </span>
                {s.constructionActive && <span style={{ fontSize: '0.625rem', fontWeight: 700, padding: '3px 9px', borderRadius: 99, color: 'var(--primary)', background: 'var(--secondary)' }}>Construction {tranchesDone}/4</span>}
                ...
                <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 12, padding: 16 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap', marginBottom: 14 }}>
                    ...
                    {s.terrainDecaisse ? (
                      ...
                      <button onClick={() => decaisserTerrain(c.id, c.name)} disabled={s.terrainMontant <= 0}
                        style={{ display: 'inline-flex', alignItems: 'center', gap: 7, padding: '9px 16px', borderRadius: 8, border: 'none', background: s.terrainMontant > 0 ? 'var(--success)' : 'var(--muted)', color: '#fff', fontSize: '0.8125rem', fontWeight: 700, cursor: s.terrainMontant > 0 ? 'pointer' : 'not-allowed' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    {FONCIER_STEPS.map((step, i) => {
                      ...
                      return (
                        <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 10px', background: done ? 'rgba(26,107,68,0.06)' : 'var(--background)', borderRadius: 8, border: '1px solid var(--border)' }}>
                  ...
                  {!s.constructionActive ? (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
                      ...
                      <button onClick={() => activateConstruction(c.id, c.name)} disabled={s.constructionMontant <= 0 || !s.terrainDecaisse}
                        style={{ display: 'inline-flex', alignItems: 'center', gap: 7, padding: '9px 16px', borderRadius: 8, border: 'none', background: s.constructionMontant > 0 && s.terrainDecaisse ? '#C8921A' : 'var(--muted)', color: '#fff', fontSize: '0.8125rem', fontWeight: 700, cursor: s.constructionMontant > 0 && s.terrainDecaisse ? 'pointer' : 'not-allowed' }}>
                    ...
                    <>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                        {CONSTRUCTION_TRANCHES.map((tr, i) => {
                          ...
                          return (
                            <div key={i} style={{ display: 'flex', gap: 12, alignItems: 'flex-start', padding: 12, background: 'var(--background)', border: '1px solid var(--border)', borderRadius: 8, flexWrap: 'wrap' }}>
                              <div style={{ width: 34, height: 34, borderRadius: 8, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: '0.8125rem', color: t.validated ? 'var(--success)' : 'var(--primary)', background: t.validated ? 'rgba(26,107,68,0.1)' : 'var(--secondary)' }}>T{i + 1}</div>
                              ...
                              <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                                {t.validated ? (
                                  <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: '0.6875rem', fontWeight: 700, color: 'var(--success)', background: 'rgba(26,107,68,0.1)', padding: '3px 9px', borderRadius: 99 }}><CheckCircle2 size={11} /> Décaissé</span>
                                ) : prevDone ? (
                                  <button onClick={() => validateTranche(c.id, i, c.name)} style={btnSm('var(--success)', 'rgba(26,107,68,0.10)')}><Send size={11} /> Décaisser</button>
                                ) : (
                                  <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: '0.6875rem', fontWeight: 700, color: '#C8921A', background: 'rgba(200,146,26,0.1)', padding: '3px 9px', borderRadius: 99 }}><Clock size={11} /> En attente</span>
                                )}
...
function btnSm(color: string, bg: string): React.CSSProperties {
  return { display: 'inline-flex', alignItems: 'center', gap: 4, padding: '5px 10px', background: bg, color, border: 'none', borderRadius: 7, fontFamily: 'var(--font-sans)', fontSize: '0.6875rem', fontWeight: 700, cursor: 'pointer', whiteSpace: 'nowrap' };
}
const btnPrimary: React.CSSProperties = { display: 'inline-flex', alignItems: 'center', gap: 6, padding: '9px 16px', borderRadius: 8, background: 'var(--primary)', color: 'var(--primary-foreground)', border: 'none', fontFamily: 'var(--font-sans)', fontSize: '0.875rem', fontWeight: 700, cursor: 'pointer' };
const btnOutline: React.CSSProperties = { display: 'inline-flex', alignItems: 'center', gap: 6, padding: '9px 14px', borderRadius: 8, background: 'transparent', color: 'var(--foreground)', border: '1px solid var(--border)', fontFamily: 'var(--font-sans)', fontSize: '0.875rem', fontWeight: 600, cursor: 'pointer' };
